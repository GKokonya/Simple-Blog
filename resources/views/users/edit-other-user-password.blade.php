...
<div class="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-lg">
      ...
      <form class="bg-white mt-6 mb-0 space-y-4 bg-white rounded-lg p-8 shadow-2xl" method="POST" action="{{ route('users.update-other-user-password', $user->id) }}">
        @method('PUT')
        @csrf

        @if (session()->has('success_user_password_message'))
        <x-success-alert>
         {{ session('success_user_password_message') }}
        </x-success-alert>
        @endif

        @if (session()->has('error_user_password_message'))
        <x-danger-alert>
         {{ session('error_user_password_message') }}
        </x-danger-alert>
        @endif
